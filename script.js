...
// Landing Page Package Loading
class LandingPagePackages {
    constructor() {
        this.supabase = null;
        this.packages = [];
        this.init();
    ...
    async init() {
        try {
            ...
            if (window.SUPABASE_CONFIG) {
                ...
                console.log('Landing page: Supabase initialized');
            }

            // Load packages
            await this.loadPackages();
        } catch (error) {
            console.error('Error initializing landing page packages:', error);
        }
    }
    ...
    async loadPackages() {
        try {
            if (this.supabase) {
                console.log('Loading packages for landing page...');
                ...
                if (data && data.length > 0) {
                    ...
                    console.log('Loaded and transformed packages for landing page:', this.packages);
                    this.displayPackages();
                } else {
                    console.log('No packages found in database for landing page');
                }
            }
        } catch (error) {
            console.error('Error loading packages for landing page:', error);
        }
    }
    ...
    displayPackages() {
        // Display birthday packages
        this.displayPackagesByService('birthday');
        this.displayPackagesByService('romantic');
        this.displayPackagesByService('theatre');
    }

    displayPackagesByService(serviceType) {
        const container = document.getElementById(`${serviceType}-packages`);
        if (!container) {
            console.log(`Container for ${serviceType} packages not found`);
            return;
        }
        ...
        if (filteredPackages.length === 0) {
            container.innerHTML = `<p class="coming">${serviceType.charAt(0).toUpperCase() + serviceType.slice(1)} Packages Coming Soon 💖</p>`;
            return;
        }
            ...
            <div class="cards-grid">
                ${filteredPackages.map(pkg => `
                    <div class="premium-card ${pkg.tier}">
                        <span class="premium-badge">${pkg.badge}</span>
                        <p class="package-type">${serviceType.charAt(0).toUpperCase() + serviceType.slice(1)} Celebration</p>
                        ...
                        <p class="addon-note">${pkg.description || 'Personalized add-ons available'}</p>
...
// Initialize landing page packages when DOM is loaded
document.addEventListener('DOMContentLoaded', function() {
    // Wait a bit for other scripts to load
    setTimeout(() => {
        window.landingPagePackages = new LandingPagePackages();
    }, 1000);
});